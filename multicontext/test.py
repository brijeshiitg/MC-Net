...
def alaska_weighted_auc(y_true, y_valid):
	...
	for idx, weight in enumerate(weights):
		...
		print(mask)
		x_padding = np.linspace(fpr[mask][0], 1, 100)
	...
	return metrics.auc(fpr,tpr),competition_metric / normalization
...
# print('test accuracy:%.2f'%(sum(test_accuracy)/len(test_accuracy)))
		...
		auc,auc_score = alaska_weighted_auc(y, new_preds)
		print('\n |AUC:%.4f|Weighted AUC:%.4f| Acc: %.3f|'%(auc,auc_score, acc))	
